...
const { logAudit } = require('../../../shared/utils/logger');
		...
		if (etpRows.length === 0) {
			await client.query(
				`INSERT INTO gepro.etp (
					...
					posicionamento_conclusivo, estimativa_quantidades, justificativa_parcelamento
				...
				[
					...
					dados.posicionamento_conclusivo || null,
					dados.estimativa_quantidades || null,
					dados.justificativa_parcelamento || null,
				],
			);
		} else {
			await client.query(
				`UPDATE gepro.etp SET
					...
					criterios_rejeicao = $22, justificativa_tecnica = $23,
					posicionamento_conclusivo = $24, estimativa_quantidades = $25,
					justificativa_parcelamento = $26, data_atualizacao = NOW()
		...
		if (trRows.length === 0) {
			await client.query(
				`INSERT INTO gepro.termo_referencia (
					...
					modelo_execucao_objeto, modelo_gestao_contrato, criterios_medicao_pagamento,
					orgaos_participantes, enderecos_entrega, requisitos_prova_conceito
				...
				[
					...
					dados.modelo_execucao_objeto || null,
					dados.modelo_gestao_contrato || null,
					dados.criterios_medicao_pagamento || null,
					dados.orgaos_participantes || null,
					dados.enderecos_entrega || null,
					dados.requisitos_prova_conceito || null,
				],
			);
		} else {
			await client.query(
				`UPDATE gepro.termo_referencia SET
					...
					clauses_rescisao = $15, modelo_execucao_objeto = $16,
					modelo_gestao_contrato = $17, criterios_medicao_pagamento = $18,
					orgaos_participantes = $19, enderecos_entrega = $20, requisitos_prova_conceito = $21, data_atualizacao = NOW()
				 WHERE instrucao_tecnica_id = $1`,
				[
					instrucaoId,
					dados.objeto || null,
...
exports.encaminhar = async (demandaId, usuarioId, ipAddress) => {
	...
	// === Geração Automática de PDF (ETP e TR) ===
	try {
		const fase2Data = await exports.obterFase2(demandaId);
		if (fase2Data.etp) {
			const { filename: etpFilename, filePath: etpFilePath } = await pdfService.gerarPDF_ETP(fase2Data.demanda, fase2Data.etp);
			await pool.query(
				`INSERT INTO gepro.documento (demanda_id, tipo, nome_documento, arquivo_path, formato) VALUES ($1, $2, $3, $4, $5)`,
				[demandaId, 'etp', etpFilename, etpFilePath, 'pdf']
			);
		}
		if (fase2Data.tr) {
			const { filename: trFilename, filePath: trFilePath } = await pdfService.gerarPDF_TR(fase2Data.demanda, fase2Data.tr);
			await pool.query(
				`INSERT INTO gepro.documento (demanda_id, tipo, nome_documento, arquivo_path, formato) VALUES ($1, $2, $3, $4, $5)`,
				[demandaId, 'tr', trFilename, trFilePath, 'pdf']
			);
		}
	} catch (pdfErr) {
		console.error("Erro na geração de PDFs (Encaminhar):", pdfErr);
		// O processo encaminhou com sucesso, mas o PDF falhou. Registramos o erro sem falhar o encaminhamento.
	}
